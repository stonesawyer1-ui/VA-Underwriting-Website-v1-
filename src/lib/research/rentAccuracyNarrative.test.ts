...
describe("buildRentAccuracyNarrative", () => {
  it("supports the buyer's number when it falls within the researched range", () => {
    ...
    expect(text.toLowerCase()).toContain("supports");
  ...
  it("says research was unavailable, honestly, when the research call itself failed", () => {
    const text = buildRentAccuracyNarrative(2300, { status: "error", message: "boom" });
    expect(text.toLowerCase()).toContain("could not independently verify");
    expect(text).toContain("2,300");
  });

  it("says research was unavailable when not configured", () => {
